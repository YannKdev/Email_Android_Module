...
frida_java_bridge_default.perform(() => {
  console.log("[*] OkHttp Interceptor (Frida 17+) loaded");
  const Interceptor2 = frida_java_bridge_default.use("okhttp3.Interceptor");
  const OkHttpBuilder = frida_java_bridge_default.use("okhttp3.OkHttpClient$Builder");
  const Buffer3 = frida_java_bridge_default.use("okio.Buffer");
  ...
  const MyInterceptor = frida_java_bridge_default.registerClass({
    name: "okhttp3.MyFridaInterceptor",
    implements: [Interceptor2],
    methods: {
      intercept(chain) {
        ...
        let respBodyStr = null;
        try {
          const peeked = response.peekBody(1024 * 1024);
          respBodyStr = peeked.string();
        } catch {
          respBodyStr = "[unreadable response]";
        }
        send({
          type: "http",
          url,
