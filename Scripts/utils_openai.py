...
def analyze_login_entry(
    ...
    If add_screenshot=False and the AI needs a screenshot,
    it returns NEED_SCREENSHOT. The caller must then retry with add_screenshot=True.

    iteration / max_iterations / already_tapped: navigation context passed to the AI
    to avoid loops and detect missing email login earlier.
    """
    ...
    # Inject navigation context at the start of the payload if available
    ...
    if etat in ("NO_LOGIN", "NO_EMAIL_LOGIN"):
        ...
        try:
            with open(dump_path, "w", encoding="utf-8") as _f:
                _f.write(data)
            logger.info(f"[{device_id}] {etat} — ui.json saved to {dump_path}")
        except Exception as _e:
            logger.warning(f"[{device_id}] {etat} — failed to save ui.json: {_e}")
...
def analyze_login_page(device_id, add_screenshot: bool = False, package_name: str = None) -> dict:
    ...
    """
    if add_screenshot:
        logger.warning("Screenshot not implemented for analyze_login_page")
    ...
    if result["email_field"] is None:
        raise ValueError("Login detected without email field")
...
def analyze_register_page(device_id, add_screenshot: bool = False, package_name: str = None) -> dict:
    ...
    """
    if add_screenshot:
        logger.warning("Screenshot not implemented for analyze_register_page")
    adb_utils.take_snapshot(device_id, screenshot=False, text_only=True)
...
def analyze_go_to_register_page(device_id, add_screenshot: bool = False, package_name: str = None) -> dict:
    ...
    """
    if add_screenshot:
        logger.warning("Screenshot not implemented for analyze_go_to_register_page")
    adb_utils.take_snapshot(device_id, screenshot=False, text_only=True)
...
def analyze_email_exists(device_id, add_screenshot: bool = False, package_name: str = None) -> dict:
    ...
    """
    if add_screenshot:
        logger.warning("Screenshot not implemented for analyze_email_exists")
    adb_utils.take_snapshot(device_id, screenshot=False, text_only=True)
    ...
    if result["etat"] not in {"INFO_EMAIL", "NO_INFO_EMAIL", "ERROR"}:
        raise ValueError("Invalid email_exists state")
...
# Token accumulator per analysis (reset before each app)
...
def _parse_json_response(raw: str) -> dict:
    ...
    try:
        ...
        raise OpenAIJSONError(f"Invalid JSON\n--- RAW ---\n{raw}") from e
    if not isinstance(parsed, dict):
        raise OpenAIJSONError("Returned JSON is not an object")
    return parsed
...
def call_openai_text_json(
    ...
    """
    Calls OpenAI with system/user separation and forces a JSON response.
    Automatically retries up to _MAX_RETRIES times on OpenAIJSONError.
    """
    last_error = None
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if not output_text:
                raise OpenAIJSONError("Empty response from model")
            ...
            if attempt < _MAX_RETRIES:
                logger.warning(f"OpenAI invalid JSON (attempt {attempt}/{_MAX_RETRIES}), retrying... {e}")
    raise last_error
...
def call_openai_image_json(
    ...
    """
    Calls OpenAI with system/user separation + image and forces a JSON response.
    Automatically retries up to _MAX_RETRIES times on OpenAIJSONError.
    """
